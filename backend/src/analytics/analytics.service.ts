...
export class AnalyticsService {
  ...
  async getVolunteerImpact(userId: string) {
    ...
    history.forEach((r) => {
      ...
      if (evt.category) {
        ...
        if (
          ...
          skillsSet.add('Mentoring');
          skillsSet.add('Public Speaking');
          sdgSet.add('SDG 4: Quality Education');
        }
        if (
          cat.includes('nature') ||
          ...
          skillsSet.add('Eco-Awareness');
          skillsSet.add('Teamwork');
          sdgSet.add('SDG 13: Climate Action');
        }
        if (cat.includes('health') || cat.includes('medical')) {
          skillsSet.add('Health Care');
          sdgSet.add('SDG 3: Good Health');
        }
        if (cat.includes('animal') || cat.includes('welfare')) {
          skillsSet.add('Animal Care');
          sdgSet.add('Empathy');
        }
        if (
          cat.includes('civic') ||
          ...
          skillsSet.add('Community Service');
          sdgSet.add('SDG 11: Sustainable Cities');
        }
        if (cat.includes('elderly') || cat.includes('elder')) {
          skillsSet.add('Caregiving');
          sdgSet.add('SDG 3: Good Health');
        }
        if (cat.includes('women') || cat.includes('empowerment')) {
          skillsSet.add('Advocacy');
          sdgSet.add('SDG 5: Gender Equality');
        }
        if (cat.includes('mental') || cat.includes('wellness')) {
          skillsSet.add('Empathy');
          sdgSet.add('SDG 3: Good Health');
        }
        if (cat.includes('youth') || cat.includes('sports')) {
          skillsSet.add('Coaching');
          sdgSet.add('SDG 3: Good Health');
        }
        if (cat.includes('art') || cat.includes('culture')) {
          skillsSet.add('Creativity');
          sdgSet.add('SDG 11: Sustainable Cities');
        }
      }
    });
    ...
    const causesBreakdown = Array.from(causesMap, ([name, value]) => ({
      name,
      value,
    }));

    // Monthly Activity
    const monthlyActivity = Array(6)
      .fill(0)
      .map((_, i) => {
        const d = new Date();
    ...
    history.forEach((r) => {
      ...
      if (status === 'completed' || status === 'checked_in') {
        ...
        if (monthIndex >= 0 && monthIndex < 6) {
          monthlyActivity[monthIndex].hours += eventHours(
            evt?.start_time,
            evt?.end_time,
          );
        }
  ...
  async getOrgAnalytics(userId: string) {
    ...
    const volunteerStats = new Map<
      string,
      { name: string; hours: number; role: string }
    >();
    ...
    events?.forEach((e) => {
      ...
      const monthEntry = monthlyGrowth.find(
        (m) => m.monthIdx === eDate.getMonth(),
      );
      ...
      regs.forEach((r) => {
        ...
        if (status === 'completed' || status === 'checked_in') {
          ...
          if (volId) {
            const existing = volunteerStats.get(volId) || {
              name: volName,
              hours: 0,
              role: 'Volunteer',
            };
    ...
    const turnoutRate =
      registeredTotal > 0
        ? Math.round((checkedInCount / registeredTotal) * 100)
        : 0;
    ...
    const eventPerformance = events
      ?.map((e) => ({
        title: e.title,
        date: e.event_date,
        volunteers: e.event_registrations.filter((r) =>
          ['completed', 'checked_in'].includes((r.status || '').toLowerCase()),
        ).length,
        success:
          e.registered_count > 0
            ? Math.round(
                (e.event_registrations.filter((r) =>
                  ['completed', 'checked_in'].includes(
    ...
    return {
      ...
      statusBreakdown: [
        ...
        {
          name: 'No-Show',
          value: Math.max(0, registeredTotal - checkedInCount - cancelledCount),
        },
