          ...
          return (
            ...
            >
              ...
              <div className="absolute top-1.5 right-1.5 flex flex-col gap-1 items-end">
                <span className="flex items-center gap-0.5 bg-primary/50 backdrop-blur-sm rounded-sm px-1.5 py-0.5 text-primary-foreground text-[9px] font-semibold leading-none">
      ...
      <div className="mb-4 px-2">
        <div className="relative group w-full">
          ...
          <input
            ...
            className="w-full h-11 pl-11 pr-10 bg-card border border-border shadow-sm rounded-2xl text-[14px] outline-none focus:border-border focus:shadow-md transition-all placeholder:text-muted-foreground"
          />
          {searchQuery && (
            <button
            ...
            {(['all', 'volunteers', 'orgs'] as const).map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab ? "bg-primary text-primary-foreground shadow-md" : "bg-card border border-border text-muted-foreground hover:bg-muted"
                )}
              >
          ...
          <div className="bg-card rounded-[20px] border border-border overflow-hidden shadow-sm">
            {displayedResults.map((item, idx) => (
              <Link
                ...
                className="flex items-center gap-3 p-3 hover:bg-muted active:bg-muted transition-colors border-b border-border last:border-0"
              >
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          {!historyLoading && history.length > 0 && (
            <div className="px-2">
              ...
              <div className="bg-card rounded-[20px] border border-border overflow-hidden shadow-sm">
                {history.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 p-3 hover:bg-muted border-b border-border last:border-0 group">
                    <Link
                      href={item.result_type === 'volunteer' ? `/volunteers/${item.result_id}` : `/organizations/${item.result_id}`}
                      className="flex items-center gap-3 flex-1 min-w-0"
                    >
                      <div className="w-9 h-9 rounded-full bg-muted overflow-hidden shrink-0 border border-border flex items-center justify-center">
                        {item.result_image
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-full hover:bg-muted transition-colors shrink-0 opacity-0 group-hover:opacity-100"
                      aria-label="Remove"
                    >
                      <X className="w-3.5 h-3.5 text-muted-foreground" />
                    </button>
              ...
              >
                {suggestionsLoading
                  ? [1, 2, 3].map((i) => (
                      <Card
                        key={i}
                        className="shrink-0 w-[140px] min-h-[192px] p-0 gap-0 animate-pulse flex flex-col"
                  ...
                  : suggestions.map((s) => {
                      ...
                      return (
                        <Card
                          key={s.user_id}
                          className="shrink-0 w-[140px] min-h-[192px] p-0 gap-0 flex flex-col"
                          style={{ scrollSnapAlign: 'start' }}
                        >
                          <Link href={`/volunteers/${s.user_id}`} className="flex flex-col items-center px-3 pt-4 gap-1.5 flex-1 min-h-0">
                            <div className="w-14 h-14 rounded-full bg-muted overflow-hidden border border-border shrink-0 flex items-center justify-center">
                              {s.avatar_url
                            ...
                            <div className="w-full text-center">
                              ...
                              {s.city && (
                                <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{s.city}</p>
                              )}
                              {s.total_hours > 0 && (
  ...
  return (
    <div className="min-h-screen bg-muted pb-20">
      <div className="max-w-2xl mx-auto px-2 sm:px-4 pt-4 pb-24">
        ...
        {/* PAGE TAB SWITCHER */}
        <div className="flex gap-1 mb-6 p-1 bg-black/5 dark:bg-white/5 rounded-full">
          <button
