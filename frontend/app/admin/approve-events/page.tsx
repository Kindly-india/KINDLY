...
export default function AdminApprovalPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-[#f8f9fa] overflow-hidden">
      ...
      {/* MOBILE HEADER */}
      <div className="lg:hidden bg-white p-4 border-b flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-emerald-600" />
            <span className="font-bold text-gray-900">Concierge Desk</span>
        </div>
        <button onClick={() => setShowSidebar(!showSidebar)} className="p-2 bg-gray-50 rounded-xl">
      ...
      <aside className={cn(
        "w-full lg:w-80 border-r border-gray-200 bg-white flex flex-col transition-all duration-300 z-40",
        ...
        <div className="p-6 border-b border-gray-100 hidden lg:block">
          <div className="flex items-center gap-2 mb-1">
            <ShieldCheck className="w-5 h-5 text-emerald-600" />
            <h1 className="text-lg font-bold text-gray-900">Concierge</h1>
          </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-50 pt-20 lg:pt-0">
          {events.length === 0 ? (
            ...
            events.map((event) => (
              <button
                ...
                className={cn(
                  "w-full text-left p-5 transition-all hover:bg-gray-50",
                  selectedEvent?.id === event.id ? "bg-emerald-50/50 border-l-4 border-emerald-500 shadow-inner" : "border-l-4 border-transparent"
                )}
              >
                <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[10px] text-gray-500 mt-1 font-medium">{event.event_date}</p>
              </button>
            ))
          )}
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-4 lg:p-10 bg-[#f8f9fa]">
        {selectedEvent ? (
          <div className="max-w-4xl mx-auto space-y-6 lg:space-y-8 pb-20">
            
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h2 className="text-xl lg:text-2xl font-black text-gray-900">Polish Submission</h2>
                <p className="text-xs lg:text-sm text-gray-500">Upgrade this event to KINDLY's club standard.</p>
              </div>
              <button 
                onClick={handleApprove}
                disabled={approving}
                className="w-full md:w-auto px-8 py-4 lg:py-3 bg-gray-900 text-white rounded-2xl font-bold shadow-2xl hover:bg-emerald-600 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
              >
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
                  ...
                  <div className="md:col-span-1 space-y-6">
                    <div>
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 block mb-2">Cover Image</label>
                        <div 
                            onClick={() => fileInputRef.current?.click()}
                            className="group relative aspect-[16/10] md:aspect-[4/5] bg-gray-200 rounded-3xl overflow-hidden border-2 border-dashed border-gray-300 hover:border-emerald-500 transition-all cursor-pointer shadow-sm"
                        >
                            {imagePreview ? (
                    ...
                    <div className="bg-white p-5 rounded-3xl border border-gray-200 shadow-sm space-y-4">
                        <div>
                            ...
                            <select 
                                className="w-full p-3 bg-gray-50 rounded-xl border-0 text-sm font-bold focus:ring-2 focus:ring-emerald-500"
                  ...
                  <div className="md:col-span-2 space-y-6">
                      <div className="bg-white p-6 lg:p-8 rounded-[32px] lg:rounded-[40px] border border-gray-200 shadow-sm space-y-6">
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Event Title</label>
                            <input 
                                className="w-full p-3 lg:p-4 bg-gray-50 rounded-2xl border-0 font-black text-lg lg:text-xl text-gray-900 focus:ring-2 focus:ring-emerald-500"
                                value={selectedEvent.title}
                                onChange={(e) => handleInputChange('title', e.target.value)}
                            />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">The Cause (Description)</label>
                            <textarea 
                                rows={6}
                                className="w-full p-3 lg:p-4 bg-gray-50 rounded-2xl border-0 text-gray-700 focus:ring-2 focus:ring-emerald-500 text-sm lg:text-base leading-relaxed"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-[32px] border border-gray-200 shadow-sm space-y-4">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Schedule & Capacity</label>
                        
                        <div className="space-y-1">
                            <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Event Date</span>
                            <input type="date" className="w-full p-3 bg-gray-50 rounded-xl border-0 text-xs font-bold" value={selectedEvent.event_date} onChange={(e) => handleInputChange('event_date', e.target.value)} />
                        </div>

                        <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                                <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Start</span>
                                <input type="time" className="w-full p-3 bg-gray-50 rounded-xl border-0 text-xs font-bold" value={selectedEvent.start_time} onChange={(e) => handleInputChange('start_time', e.target.value)} />
                            </div>
                            <div className="space-y-1">
                                <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">End</span>
                                <input type="time" className="w-full p-3 bg-gray-50 rounded-xl border-0 text-xs font-bold" value={selectedEvent.end_time} onChange={(e) => handleInputChange('end_time', e.target.value)} />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                                <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Total Slots</span>
                            ...
                            <div className="space-y-1">
                                <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Min Age</span>
                                <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl">
                    ...
                    {/* Contact & Deadline */}
                    <div className="bg-white p-6 rounded-[32px] border border-gray-200 shadow-sm space-y-4">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Communication & Deadlines</label>
                        
                        <div className="space-y-1">
                            <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Registration Deadline</span>
                            <div className="relative">
                                <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                                <input 
                                    type="datetime-local"
                                    className="w-full p-3 pl-10 bg-gray-50 rounded-xl border-0 text-xs font-bold focus:ring-2 focus:ring-emerald-500" 
                        ...
                        <div className="space-y-1">
                            <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Point of Contact (Admin Override)</span>
                            <div className="relative">
                        ...
                        <div className="space-y-1">
                            <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Exact Venue Location</span>
                            <div className="relative">
                    ...
                    {/* Clothing & Items */}
                    <div className="col-span-1 md:col-span-2 bg-white p-6 rounded-[32px] border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Dress Code</span>
                            <input className="w-full p-3 bg-gray-50 rounded-xl border-0 text-xs font-bold" value={selectedEvent.dress_code || ""} onChange={(e) => handleInputChange('dress_code', e.target.value)} placeholder="e.g. White T-shirt" />
                        </div>
                        <div className="space-y-1">
                            <span className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Things to Bring</span>
                            <input className="w-full p-3 bg-gray-50 rounded-xl border-0 text-xs font-bold" value={selectedEvent.things_to_bring || ""} onChange={(e) => handleInputChange('things_to_bring', e.target.value)} placeholder="e.g. Water bottle" />
                        </div>
                    </div>
              </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-gray-300 px-6 text-center">
            ...
            <p className="text-xs text-gray-400 mt-2">All submissions from Nashik organizations will appear here.</p>
          </div>
        )}
      </main>
    </div>
