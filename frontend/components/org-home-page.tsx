...
export function OrgHomePage() {
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-[#f0f7ff] dark:from-[#f0f7ff]/10 via-[#f5faff] dark:via-[#f5faff]/10 to-[#f0fdf4] dark:to-[#f0fdf4]/10 py-8 md:py-16 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 text-center relative">
          ...
          <h1 className="text-[24px] md:text-[56px] font-bold text-foreground tracking-tight leading-tight">
            Welcome back,{" "}
          ...
          <p className="text-[14px] md:text-[19px] font-semibold text-foreground mt-2 md:mt-3">
            Ready to make an impact in your society?
          </p>

          <div className="flex justify-center gap-2 md:gap-4 mt-6 md:mt-8">
            <div className="bg-card rounded-xl px-3 md:px-6 py-3 md:py-4 shadow-sm border border-border">
              <p className="text-[18px] md:text-[28px] font-bold text-[#0066cc]">
                {stats.totalHours.toLocaleString()}
              </p>
              <p className="text-[10px] md:text-[12px] text-muted-foreground">Total Hours</p>
            </div>
            <div className="bg-card rounded-xl px-3 md:px-6 py-3 md:py-4 shadow-sm border border-border">
              <p className="text-[18px] md:text-[28px] font-bold text-[#10b981]">
                {stats.activeVolunteers.toLocaleString()}
              </p>
              <p className="text-[10px] md:text-[12px] text-muted-foreground">Active Volunteers</p>
            </div>
            <div className="bg-card rounded-xl px-3 md:px-6 py-3 md:py-4 shadow-sm border border-border">
              <p className="text-[18px] md:text-[28px] font-bold text-[#f59e0b]">
                {stats.eventsHosted}
              </p>
              <p className="text-[10px] md:text-[12px] text-muted-foreground">Events Hosted</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-card py-6 md:py-10 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <h2 className="text-[18px] md:text-[24px] font-bold text-foreground mb-4">Quick Actions</h2>
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            <Link href="/org-events/create" className="bg-gradient-to-br from-[#f0fdf4] dark:from-[#f0fdf4]/10 to-[#dcfce7] dark:to-[#dcfce7]/10 rounded-xl md:rounded-2xl p-4 md:p-6 hover:shadow-md transition-all group border border-[#bbf7d0]">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-card shadow-sm flex items-center justify-center mb-3 group-hover:scale-105 transition-transform">
                <Plus className="w-5 h-5 md:w-6 md:h-6 text-emerald-600" />
              </div>
              <h3 className="text-[14px] md:text-[17px] font-semibold text-foreground">Create Event</h3>
              <p className="text-[11px] md:text-[13px] text-muted-foreground mt-0.5">Draft a new volunteer drive</p>
            </Link>
            <Link href="/org-events?tab=active" className="bg-gradient-to-br from-[#fff7ed] dark:from-[#fff7ed]/10 to-[#ffedd5] dark:to-[#ffedd5]/10 rounded-xl md:rounded-2xl p-4 md:p-6 hover:shadow-md transition-all group border border-[#fed7aa]">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-card shadow-sm flex items-center justify-center mb-3 group-hover:scale-105 transition-transform">
                <Megaphone className="w-5 h-5 md:w-6 md:h-6 text-[#f59e0b]" />
              </div>
              <h3 className="text-[14px] md:text-[17px] font-semibold text-foreground">Broadcast</h3>
              <p className="text-[11px] md:text-[13px] text-muted-foreground mt-0.5">Message all volunteers</p>
            </Link>
          </div>
        </div>
      </section>
...
{/* Your Events Section (Updated with 16:9 and Fixed Heights) */}
      <section className="bg-muted py-6 md:py-12">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h2 className="text-[20px] md:text-[36px] font-bold text-foreground tracking-tight">Your Events</h2>
              <p className="text-[12px] md:text-[15px] text-muted-foreground mt-0.5">Manage and track your volunteer drives</p>
            </div>
            ...
            <>
                ...
                {events.filter(e => e.status !== 'cancelled').map((event) => (
                  ...
                  >
                    ...
                    <div className="relative aspect-video aspect-[16/9] w-full overflow-hidden bg-muted shrink-0">
                      ...
                      <div className={cn("absolute top-2 right-2 px-2 py-0.5 rounded text-[9px] md:text-[11px] font-semibold capitalize z-10", event.status === "published" ? "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700" : "bg-amber-100 dark:bg-amber-500/15 text-amber-700")}>
                        {event.status}
                      </div>
                    ...
                    <div className="p-3 md:p-4 flex flex-col flex-1">
                      <h3 className="text-[13px] md:text-[15px] font-semibold text-foreground mb-1.5 line-clamp-1">
                        {event.title}
                      </h3>
              ...
              {events.length > 4 && (
                <div className="flex items-center justify-center gap-1.5 md:gap-2 mt-6 md:mt-10">
                  <button onClick={() => scrollEvents("left")} className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-border flex items-center justify-center hover:bg-card transition-colors">
                    <ChevronLeft className="w-4 h-4 md:w-5 md:h-5 text-foreground" />
                  </button>
                  <div className="flex items-center gap-1">
                    {[1, 2, 3].map((page) => (
                      <button key={page} className={cn("w-7 h-7 md:w-8 md:h-8 rounded-full text-[11px] md:text-[13px] font-medium transition-colors", page === 1 ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-card")}>
                        {page}
                      </button>
                    ))}
                  </div>
                  <button onClick={() => scrollEvents("right")} className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-border flex items-center justify-center hover:bg-card transition-colors">
      ...
      <section className="bg-card py-6 md:py-10">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <h2 className="text-[18px] md:text-[24px] font-bold text-foreground mb-4">Recent Activity</h2>
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="bg-muted rounded-xl md:rounded-2xl overflow-hidden divide-y divide-white">
              {recentActivity.map((activity) => (
                <div key={activity.id} className="flex items-start gap-3 p-3.5 md:p-4 bg-card">
                  <div
                    className={cn(
                      "w-8 h-8 md:w-10 md:h-10 rounded-full shrink-0 flex items-center justify-center",
                      activity.type === "publish"
                        ? "bg-gradient-to-br from-[#e0f2fe] dark:from-[#e0f2fe]/10 to-[#bae6fd] dark:to-[#bae6fd]/10"
                        : activity.type === "checkin"
                          ? "bg-gradient-to-br from-[#dcfce7] dark:from-[#dcfce7]/10 to-[#bbf7d0] dark:to-[#bbf7d0]/10"
                          : "bg-gradient-to-br from-[#f3e8ff] dark:from-[#f3e8ff]/10 to-[#e9d5ff] dark:to-[#e9d5ff]/10",
                    )}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] md:text-[15px] text-foreground leading-snug">{activity.text}</p>
                    <p className="text-[11px] md:text-[13px] text-muted-foreground mt-0.5">{timeAgo(activity.timestamp)}</p>
                  </div>
                </div>
              ))}
            </div>
